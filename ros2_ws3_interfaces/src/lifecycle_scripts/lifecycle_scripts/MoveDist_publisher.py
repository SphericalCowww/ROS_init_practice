...
#######################################################################################################################
class MoveDist_publisherNode(LifecycleNode):
    def __init__(self):
        super().__init__("MoveDist_publisher")
        self.get_logger().info("MoveDist_publisherNode: initializing")
        self.number_            = 1
    ...
    def on_configure(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: configuring")
    ...
    def on_cleanup(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: cleaning up")
        self.cleanup_()
        return TransitionCallbackReturn.SUCCESS
    def on_activate(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: activating")
        self.number_timer_.reset()
        return super().on_activate(statePre)
    def on_deactivate(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: deactivating")
        self.number_timer_.cancel()
        return super().on_deactivate(statePre)
    def on_shutdown(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: shutting down")
        self.cleanup_()
        return TransitionCallbackReturn.SUCCESS
    def on_error(self, statePre: LifecycleState):
        self.get_logger().info("MoveDist_publisherNode: deadly error occured, shutting down")
        self.cleanup_()
